...
__attribute__((used,noinline)) static void loadExtraSetting(void) {
 ...
 else strcat(settingsPath,"\\NFSU2-NativeTrax-LGU.ini");
...
static void __attribute__((fastcall)) applyPlaylist(void *player,void *unused,unsigned menu,unsigned race,int menus,int races,int shuffle) {
 ...
 ((void (__attribute__((thiscall)) *)(void*,unsigned,unsigned,int,int,int))0x46bb40)(player,menu,race,menus,races,shuffle);
...
static int __attribute__((fastcall,noinline)) chooseTrack(void *self,void *unused) {
 ...
 int chosen=-1;
 if(*(int*)(p+0x9c)) {
  unsigned pick=((unsigned (__cdecl *)(unsigned))0x43c220)(count);
...
static void __cdecl showAutomatic(Metadata *entry) {
 ...
 ((void (__cdecl *)(Metadata*))0x4ac960)(validExtra(custom)?&metadata[custom]:entry);
}
...
static int dispatchLogged(unsigned handle,unsigned event,const char *origin) {
 int result=((int (__cdecl *)(unsigned,unsigned))0x7399b0)(handle,event);
...
static void __attribute__((fastcall,noinline)) previewTrack(void *player,void *unused,int index) {
 ...
 ((void (__attribute__((thiscall)) *)(void*,int))0x47af80)(player,validExtra(customPreview)?0:index);
 customPreview=previous;
 if(validExtra(index))((void (__cdecl *)(Metadata*))0x4ac960)(&metadata[index]);
}
static int __cdecl routeStatus(unsigned handle,int *status) {
 int result=((int (__cdecl *)(unsigned,int*))0x739100)(handle,status);
...
static void __attribute__((fastcall)) updateSettings(void *self,void *unused) {
 ...
 ((void (__attribute__((thiscall)) *)(void*))0x479210)(self);
...
__attribute__((used,noinline)) static void __attribute__((fastcall)) sizeAddedBanner(
 ...
 (void)unused;
 ((void (__attribute__((thiscall)) *)(void*,const char*,const char*,const char*))0x4ac740)(self,title,artist,album);
...
#include "patches.h"
__attribute__((used,noinline)) static void configurePatchCounts(void) {for(unsigned i=0;i<sizeof(patches)/sizeof(*patches);i++){if(patches[i].address==0x54ba78)patches[i].value=trackCount;if(patches[i].address==0x53de42)patches[i].value=trackCount*8;}}
...
__declspec(dllexport) BYTE lguMapHash[32]={0x4c,0x47,0x55,0xff};
static int verifyFiles(void) {
 char path[MAX_PATH];
 static const BYTE exeHash[32]={0x82,0x88,0x1d,0x60,0x7e,0xa8,0xd9,0x0a,0x28,0x98,0xba,0x9a,0x6e,0xd0,0x6d,0xa1,0xff,0x6b,0x56,0x24,0x3f,0x2e,0x91,0x46,0xdf,0x30,0x22,0x6f,0xff,0x69,0x57,0x4b};
 ...
 char *slash=strrchr(path,'\\');if(!slash||slash-path+32>=MAX_PATH)return 0;
 strcpy(slash+1,"PFDATA\\MusicSFx.mus");if(!verifyHash(path,packageConfig.archive))return 0;
...
BOOL WINAPI DllMain(HINSTANCE module,DWORD reason,LPVOID reserved) {
 ...
 DWORD tempLen=GetTempPathA(MAX_PATH,logpath);if(tempLen&&tempLen+18<MAX_PATH)strcat(logpath,"NativeTrax-LGU.log");else strcpy(logpath,"NativeTrax-LGU.log");
 ...
 menuTrampoline=trampoline(0x54b250,7);callbackTrampoline=trampoline(0x53de10,7);
 decoderTrampoline=trampoline(0x728dc6,6);
